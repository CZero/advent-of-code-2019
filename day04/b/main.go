...
// validate checks if num is a possible code: Never decreases and has at least 1 pair of the same adjacent digits, the pair can't be longer then 2
func validate(input int) bool {
	...
	for i := 1; i < len(number); i++ {
		...
		if libaoc.SilentAtoi(number[i]) == libaoc.SilentAtoi(number[i-1]) {
			if i+1 < len(number) {
				if libaoc.SilentAtoi(number[i]) == libaoc.SilentAtoi(number[i+1]) {
					continue
				}
			...
			if i-2 >= 0 {
				if libaoc.SilentAtoi(number[i]) == libaoc.SilentAtoi(number[i-2]) {
					continue
				}
			}
			pairs++ // This was a valid pair, as there were no more than 2 digits in it
		}
	}
	if pairs == 0 {
		return false
